var Engine = (function () {
	...
	function Sprite$2(args, eventList, renderer, loader, touchSystem, sprites) {
	    ...
	    this.costume_id = util$2.isNumeric(args.costume_id) ? args.costume_id : 0;
	    this.costumes = [].concat(args.costumes); // Deal with single string
	    this.hidden = !!args.hidden; // change to boolean
	    this.rotation_style = ['full','flipped','fixed'].includes(args.rotation_style) ? args.rotation_style : 'full'; 
	    this.width = 1;
	    this.height = 1;
	...
	Sprite$2.prototype = {
	    ...
	    step_forward: function (distance) {
	    ...
	    next_costume: function () {
	        this.costume_id += 1;
	        if(this.costume_id >= this.costumes.length) {
	            this.costume_id = 0;
	        }
	    },

	    bounce_edge: function () {
	    ...
	    get_costume_image: function () {
	        var id = this.costumes[this.costume_id];
	    ...
	    _updateSize: function () {
	        var img = this.get_costume_image();
	    ...
	    _updateFrames: function () {
	        ...
	        if(animate.frames.length > 0) {
	            ...
	            if(now >= animate.timer + 1000 / animate.rate) {
	                animate.timer = now;
	                this.costume_id = animate.frames.shift();
	...
	Renderer$2.prototype = {
	    ...
	    drawInstance: function(sprite){
	        ...
	        var ctx = this.ctx;
	        var img = sprite.get_costume_image();
	        var rad = util$1.degreeToRad(sprite.direction - 90);

	        ctx.globalAlpha = sprite.opacity;
	        if (sprite.rotation_style === 'flipped') {
	        ...
	        if(sprite.rotation_style === 'fixed') {
	            var rad = 0;
	        }
	    ...
	    draw_backdrop: function (src, x, y, width, height){
	        if(src.includes('.')) {
	...
	function IO$1(canvas, settings){
	    ...
	    // 滑鼠/觸控指標狀態
	    var cursor    = this.cursor    = { x: 0, y: 0, is_down: false, left: false, right: false };
	    // 最後一次點擊事件的位置
	    ...
	    canvas.addEventListener("mouseup", function(e){
	        ...
	        cursor.is_down = cursor.left || cursor.right; // 只有當左右鍵都放開時才算放開
	        // 根據縮放比例校正座標
	...
	Pen$1.prototype = {
	    ...
	    draw_shapes: function () {
	        ...
	        for(let i=0; i<this.shapes.length; i++) {
	            ...
	            if (s.type == 'line') {
	                this._draw_line(s.x1,s.y1,s.x2,s.y2);
	            }
	            else if (s.type == 'circle') {
	                this._draw_circle(s.x, s.y, s.r);
	            }
	            else if (s.type == 'triangle') {
	                this._draw_triangle(s.x1, s.y1, s.x2, s.y2, s.x3, s.y3);
	            }
	            else if (s.type == 'rect') {
	                this._draw_rect(s.x, s.y, s.w, s.h);
	            }
	            else if (s.type == 'polygon') {
	                this._draw_polygon.apply(this, s.points);
	            }
	        }
	        this.shapes=[];
	    },
	    ...
	    draw_texts: function () {
	        for(let i=0; i<this.texts.length; i++) {
	            let t = this.texts[i];
	            this._draw_text(t.text, t.x, t.y, t.color, t.size, t.font);
	        }
	        this.texts=[];
	    },

	    draw_text: function (text, x, y, color ,size, font) {
	        x = x == undefined ? 10 : x;
	        y = y == undefined ? 10 : y;
	        color = color || 'black';
	        size = size || 16;
	        ...
	        if (!this.settings.autoRendering) return this._draw_text(text, x, y, color ,size, font);
	        // 如果是 autoRener，存起來
	        this._addText({
	            text: text,
	            x: x,
	    ...
	    draw_line: function (x1, y1, x2, y2) {
	        if (!this.settings.autoRendering) return this._draw_line(x1, y1, x2, y2);
	        var s = {};
	        s.x1 = x1;
	        s.y1 = y1;
	        s.x2 = x2;
	    ...
	    draw_circle: function (x, y ,r) {
	        if (!this.settings.autoRendering) return this._draw_circle(x, y ,r);
	        var s = {};
	        s.x = x;
	        s.y = y;
	        s.r = r;
	    ...
	    draw_triangle: function (x1, y1, x2, y2, x3, y3) {
	        if (!this.settings.autoRendering) return this._draw_triangle(x1, y1, x2, y2, x3, y3);
	        var s = {};
	        s.x1 = x1;
	        s.y1 = y1;
	        s.x2 = x2;
	    ...
	    draw_rect: function (x, y, width, height) {
	        if (!this.settings.autoRendering) return this._draw_rect(x, y, width, height);
	        var s = {};
	        s.x = x;
	        s.y = y;
	        s.w = width;
	    ...
	    draw_polygon: function () {
	        if (!this.settings.autoRendering) return this._draw_polygon.apply(this, arguments);
	        var s = {};
	    ...
	    _draw_text: function (text, x, y, color ,size, font) {
	    ...
	    _draw_line: function (x1, y1, x2, y2) {
	        if(!this.settings.autoRendering) this._setPenAttr();
	        this.ctx.beginPath();
	    ...
	    _draw_triangle: function (x1, y1, x2, y2, x3, y3) {
	        if(!this.settings.autoRendering) this._setPenAttr();
	        this.ctx.beginPath();
	        this.ctx.moveTo(x1, y1);
	        this.ctx.lineTo(x2, y2);
	...
	TouchSystem$1.prototype = {
	    ...
	    getBoxOf: function (sprite) {
	        ...
	        if (sprite.direction === 90 || sprite.direction === 270 || sprite.rotation_style !== 'full') {
	...
	function engine(canvasId, debugMode){
	    ...
	    function gameloop () {
	        ...
	        if(settings.autoRendering){
	            renderer.draw_backdrop(background.path, background.x, background.y, background.w, background.h);
	            pen.draw_shapes();
	    ...
	    function set_background (path, x, y, w, h) {
	    ...
	    var proxy = {
	        ...
	        create_sound: sound.play.bind(sound),
	        set_background: set_background,
	        set_backdrop: set_background,
	        inspector: inspector,
	        forever: forever,
	        update: forever,
	        always: forever,
	        ...
	        pen: pen,
	        print: pen.draw_text.bind(pen),
	        draw_text: pen.draw_text.bind(pen),
	        draw_backdrop: renderer.draw_backdrop.bind(renderer),
	        draw_background: renderer.draw_backdrop.bind(renderer),
	        draw_sprites: renderer.draw_sprites.bind(renderer),
